...
void StatisticsSampler::saveToFile(int timeStep)
{
    ...
    if (m_initialSample) {
        m_outFile.open("halfDensitySpheresNc20T15.txt", fstream::out | fstream::trunc);
    ...
    m_outFile.open("halfDensitySpheresNc20T15.txt", fstream::out | fstream::app);
...
void StatisticsSampler::sampleRadialDistribution(int numberOfBins) {
    if (m_firstRadial) {
        ...
        m_radialDistFile.open("radDistNc10T15ThermoBins30.txt", fstream::out | fstream::trunc);
    ...
    m_radialDistFile.open("radDistNc10T15ThermoBins30.txt", fstream::out | fstream::app);
    for (int bin=0; bin < numberOfBins; bin++) {
