...
double ManyNeighbourNN::network2(arma::mat inputVector) {
    // input vector is a 1xinputs vector
...
void ManyNeighbourNN::calculateForces() {
    ...
    std::vector<std::vector<double> > drNeighbours(m_numberOfInputs, std::vector<double>(3));
    for (int i=0; i < m_system->atoms().size(); i++) {
        ...
        for (int j=0; j < m_cellList->getNeighbours()[i].size(); j++) {
            ...
            // add distances to input vector as long as there are inputs left to fill
            if (inputCount < m_numberOfInputs) {
                ...
                if (dr2 < m_rCutSquared) {
                    ...
                    distanceNeighbours(0, inputCount)   = dr[0];
                    distanceNeighbours(0, inputCount+1) = dr[1];
                    distanceNeighbours(0, inputCount+2) = dr[2];
                    distanceNeighbours(0, inputCount+3) = distance;
                    inputCount += 4;
                }
            }

            // if out of neighbours, break loop
            else
                break;
        }

        //cout <<  m_cellList->getNeighbours()[i].size() << endl;
        //cout << inputCount << endl;

        // if not all inputs are filled: fill the rest with r=cut and random forces
        for (int k=inputCount; k < m_numberOfInputs; k += 4) {
            distanceNeighbours(0, k)    = 1.87;
            distanceNeighbours(0, k+1)  = 1.32;
            distanceNeighbours(0, k+2)  = 1.006;
            distanceNeighbours(0, k+3)  = m_rCut;
        }

        // after making input vector, compute total energy and force
        arma::mat energyAndForce = network(distanceNeighbours);
        //cout << energyAndForce(0,1) << endl;
        potentialEnergy += energyAndForce(0,3);

        vec3 forceOnAtom;
        forceOnAtom[0] = energyAndForce(0,0);
        forceOnAtom[1] = energyAndForce(0,1);
        forceOnAtom[2] = energyAndForce(0,2);
        ...
        for (int k=0; k < m_numberOfInputs; k++) {
